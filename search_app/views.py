...
def process_query(request):
    if request.method == 'POST':
        ...
        doc_texts = ['i loved you ethiopia, stored elements in Compress find Sparse Ethiopia is the greatest country in the world of nation at universe',
...
def get_similar_articles(query, doc_texts, df, titles, n):
    ...
    for i in range(len(doc_texts)):
        ...
        titl[i] = np.dot(df.loc[:, i].values, q_vec) / (np.linalg.norm(df.loc[:, i]) * np.linalg.norm(q_vec))
